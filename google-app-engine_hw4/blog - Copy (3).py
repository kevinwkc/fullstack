...
class BlogHandler(webapp2.RequestHandler):
    ...
    def getName(self):
        if not self.user:
            ...
            return User.by_id(int(self.user)).name
...
class NewComment(BlogHandler):
    ...
    def post(self):
        ...
        post_id = self.request.get('id')
        if  content:
            p = Comment(parent = blog_key(), user=name, content = content, post_id=int(post_id))
            p.put()     
...
class LikePage(BlogHandler):
    ...
    def post(self):
        ...
        if name != p.user and name not in p.like:
          p.like.append(name)
          p.put()
          self.redirect('/blog')
        else:
...
class UnLikePage(BlogHandler):
    ...
    def post(self):
        ...
        if  name != p.user and  name not in p.unlike:
...
class DeletePost(BlogHandler):
    ...
    def post(self):
        ...
        if  name == post.user:
            ...
            self.getComment(post.key().id()).delete()
            post.delete()
